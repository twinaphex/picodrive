...
#else
static void draw_dirlist(char *curdir, struct my_dirent **namelist,
	int n, int sel, int show_help)
#endif
...
#else
	darken_ptr = (short *)g_menuscreen_ptr + g_menuscreen_w * ((max_cnt/2 * me_sfont_h)-1);
#endif
...
#else
		darken_ptr = (short *)g_menuscreen_ptr
			+ g_menuscreen_w * (g_menuscreen_h - me_sfont_h * 7 / 2);
		menu_darken_bg(darken_ptr, darken_ptr,
			g_menuscreen_w * (me_sfont_h * 7 / 2), 1);
#endif
...
static int scandir_filter(const struct my_dirent *ent)
{
	...
	for (i = 0; i < sizeof(filter_exts_internal)/sizeof(filter_exts_internal[0]); i++)
	{
		if (strcasecmp(p, filter_exts_internal[i]) == 0) return 0;
	}
...
#else
static int dirent_seek_char(struct my_dirent **namelist, int len, int sel, char c)
#endif
...
static const char *menu_loop_romsel(char *curr_path, int len,
	const char **filter_exts,
	int (*extra_filter)(struct my_dirent **namelist, int count,
			    const char *basedir))
#endif
{
...
#else
	int (*filter)(const struct my_dirent *);
	struct my_dirent **namelist = NULL;
#endif
