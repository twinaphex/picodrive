...
#ifdef _ASM_DRAW_C
void BackFillFull_asm(int reg7);
void DrawLayerFull_asm(int plane, int *hcache, int planestart, int planeend);
void DrawTilesFromCacheF_asm(int *hc);
void DrawWindowFull_asm(int start, int end, int prio);
void DrawSpriteFull_asm(unsigned int *sprite);
#endif
...
static void DrawWindowFull_c(int start, int end, int prio)
{
...
static void DrawLayerFull_c(int plane, int *hcache, int planestart, int planeend)
{
	struct PicoVideo *pvid=&Pico.video;
...
static void DrawTilesFromCacheF_c(int *hc)
{
	int code, addr, zero = 0;
...
static void DrawSpriteFull_c(unsigned int *sprite)
{
...
static void BackFillFull_c(int reg7)
{
	unsigned int back;
...
static void DrawDisplayFull_c(void)
{
	...
	if (HighCache2B[1]) DrawTilesFromCacheF_c(HighCache2B);
	if (HighCache2A[1]) DrawTilesFromCacheF_c(HighCache2A);
	if (PicoDrawMask & PDRAW_LAYERA_ON) switch (hvwin)
	{
		case 4:
		// fullscreen window
...
PICO_INTERNAL void PicoFrameFull()
{
	...
	// Draw screen:
	BackFillFull_asm(Pico.video.reg[7]);
