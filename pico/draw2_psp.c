...
void PicoDraw2SetOutBuf(void *dest, int increment)
{
  if (dest != NULL) {
	...
	Pico.est.Draw2FB = PicoDraw2FBBase;
  }
  else {
...
static void DrawWindowFull(int start, int end, int prio, struct PicoEState *est)
{
	...
	unsigned char *scrpos = Pico.est.Draw2FB;
...
static void DrawTilesFromCacheF_c(int *hc, struct PicoEState *est)
{
	int code, addr, zero = 0;
...
static void DrawSpriteFull_c(unsigned int *sprite, struct PicoEState *est)
{
...
static void DrawAllSpritesFull_c(int prio, int maxwidth)
{
	struct PicoVideo *pvid=&Pico.video;
	...
	for (i--; i >= 0; i--)
	{
		DrawSpriteFull_c(sprites[i], &Pico.est);
	}
}

static void DrawAllSpritesFull_asm(int prio, int maxwidth)
{
	struct PicoVideo *pvid=&Pico.video;
	int table=0,maskrange=0;
	int i,u,link=0;
	...
	for (i-- ;i>=0; i--)
	{
		DrawSpriteFull_asm(sprites[i]);
	}
}


static void BackFillFull_c(void *dst, int reg7)
{
	unsigned int back;
	...
	memset32(dst, back, line_widht*(8+(END_ROW-START_ROW)*8)/4);
}
#endif

static void DrawDisplayFull_c(void)
{
	...
	if (!(pvid->debug_p & PVD_KILL_B))
		DrawLayerFull_c(1, HighCache2B, START_ROW, (maxcolc<<16)|END_ROW, est);
	if (!(pvid->debug_p & PVD_KILL_A)) switch (hvwin)
	{
		case 4:
		// fullscreen window
		DrawWindowFull_c(START_ROW, (maxcolc<<16)|END_ROW, 0, est);
		break;

		case 3:
		// we have plane A and both v and h windows
		DrawLayerFull_c(0, HighCache2A, planestart, planeend, est);
		DrawWindowFull_c( winstart&~0xff0000, (winend&~0xff0000)|(maxcolc<<16), 0, est); // h
		DrawWindowFull_c((winstart&~0xff)|START_ROW, (winend&~0xff)|END_ROW, 0, est);    // v
		break;
		...
		DrawLayerFull_c(0, HighCache2A, planestart, planeend, est);
		DrawWindowFull_c(winstart, winend, 0, est);
		break;

		default:
		// fullscreen plane A
		DrawLayerFull_c(0, HighCache2A, START_ROW, (maxcolc<<16)|END_ROW, est);
		break;
	}
	if (!(pvid->debug_p & PVD_KILL_S_LO))
		DrawAllSpritesFull_c(0, maxw);

	if (HighCache2B[1]) DrawTilesFromCacheF_c(HighCache2B, est);
	if (HighCache2A[1]) DrawTilesFromCacheF_c(HighCache2A, est);
	if (!(pvid->debug_p & PVD_KILL_A)) switch (hvwin)
	{
		case 4:
		// fullscreen window
		DrawWindowFull_c(START_ROW, (maxcolc<<16)|END_ROW, 1,est);
		break;

		case 3:
		// we have plane A and both v and h windows
		DrawWindowFull_c( winstart&~0xff0000, (winend&~0xff0000)|(maxcolc<<16), 1,est); // h
		DrawWindowFull_c((winstart&~0xff)|START_ROW, (winend&~0xff)|END_ROW, 1,est);    // v
		break;

		case 2:
		case 1:
		// both window and plane A visible, window is vertical XOR horizontal
		DrawWindowFull_c(winstart, winend, 1, est);
		break;
	}
	if (!(pvid->debug_p & PVD_KILL_S_HI))
		DrawAllSpritesFull_c(1, maxw);
}

static void DrawDisplayFull_semi_asm(void)
{
	struct PicoVideo *pvid=&Pico.video;
	int win, edge=0, hvwin=0; // LSb->MSb: hwin&plane, vwin&plane, full
	int planestart=START_ROW, planeend=END_ROW; // plane A start/end when window shares display with plane A (in tile rows or columns)
	...
	if (PicoDrawMask & PDRAW_LAYERA_ON) switch (hvwin)
	{
		case 4:
		// fullscreen window
		DrawWindowFull_c(START_ROW, (maxcolc<<16)|END_ROW, 1);
		break;

		case 3:
		// we have plane A and both v and h windows
		DrawWindowFull_c( winstart&~0xff0000, (winend&~0xff0000)|(maxcolc<<16), 1); // h
		DrawWindowFull_c((winstart&~0xff)|START_ROW, (winend&~0xff)|END_ROW, 1);    // v
		break;

		case 2:
		case 1:
		// both window and plane A visible, window is vertical XOR horizontal
		DrawWindowFull_c(winstart, winend, 1);
		break;
	}
	if (PicoDrawMask & PDRAW_SPRITES_HI_ON)
		DrawAllSpritesFull_asm(1, maxw);
}

static void DrawDisplayFull_asm(void)
{
	struct PicoVideo *pvid=&Pico.video;
	int win, edge=0, hvwin=0; // LSb->MSb: hwin&plane, vwin&plane, full
	int planestart=START_ROW, planeend=END_ROW; // plane A start/end when window shares display with plane A (in tile rows or columns)
	...
	if (PicoDrawMask & PDRAW_LAYERA_ON) switch (hvwin)
	{
		case 4:
		// fullscreen window
			DrawWindowFull_asm(START_ROW, (maxcolc<<16)|END_ROW, 0);
		break;

		case 3:
		// we have plane A and both v and h windows
		DrawLayerFull_asm(0, HighCache2A, planestart, planeend);
		DrawWindowFull_asm( winstart&~0xff0000, (winend&~0xff0000)|(maxcolc<<16), 0); // h
		DrawWindowFull_asm((winstart&~0xff)|START_ROW, (winend&~0xff)|END_ROW, 0);    // v
		break;
		...
		// both window and plane A visible, window is vertical XOR horizontal
		DrawLayerFull_asm(0, HighCache2A, planestart, planeend);
		DrawWindowFull_asm(winstart, winend, 0);
		break;
	...
	if (PicoDrawMask & PDRAW_LAYERA_ON) switch (hvwin)
	{
		case 4:
		// fullscreen window
		DrawWindowFull_asm(START_ROW, (maxcolc<<16)|END_ROW, 1);
		break;

		case 3:
		// we have plane A and both v and h windows
		DrawWindowFull_asm( winstart&~0xff0000, (winend&~0xff0000)|(maxcolc<<16), 1); // h
		DrawWindowFull_asm((winstart&~0xff)|START_ROW, (winend&~0xff)|END_ROW, 1);    // v
		break;

		case 2:
		case 1:
		// both window and plane A visible, window is vertical XOR horizontal
		DrawWindowFull_asm(winstart, winend, 1);
		break;
	}
	if (PicoDrawMask & PDRAW_SPRITES_HI_ON)
		DrawAllSpritesFull_asm(1, maxw);
...
PICO_INTERNAL void PicoFrameFull()
{
	...
	// Draw screen:
	BackFillFull_asm(Pico.est.Draw2FB, Pico.video.reg[7]);
